...
export class CautelarMateriaisComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //Autenticacao
    this.authService.getCurrentUser().subscribe(nome => {
  ...
  salvar(): void {
    if (this.form.valid && this.nomeUsuario) {
      ...
      // Trate o caso em que o formulário não é válido ou nomeUsuario não está disponível
      console.error('Formulário inválido ou usuário não autenticado.');
    }
  }
